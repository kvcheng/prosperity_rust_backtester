...
class AshCoatedOsmiumStrategy:
    ...
    def _trend_regime(self):
        ...
        if short_ma <= long_ma - 1.0 and momentum <= -2.0:
            return -2
    ...
    def take(self, order_depth, position: int, fair: float, z: float, regime: int, target_position: int):
        ...
        if target_position < position and order_depth.buy_orders:
            ...
            for bid in sorted(order_depth.buy_orders.keys(), reverse=True):
                ...
                qty = min(bid_volume, remaining_to_sell, self.MAX_TAKE_PER_LEVEL)
                if qty > 0 and bid >= fair - 1:
                    orders.append(Order("ASH_COATED_OSMIUM", bid, -qty))
                    position -= qty
                    remaining_to_sell -= qty

        if target_position > position and order_depth.sell_orders:
            ...
            for ask in sorted(order_depth.sell_orders.keys()):
                ...
                qty = min(ask_volume, remaining_to_buy, self.MAX_TAKE_PER_LEVEL)
        ...
        if order_depth.sell_orders:
            for ask in sorted(order_depth.sell_orders.keys()):
                ...
                if ask <= fair - band and position < limit and regime >= 0:
                    qty = min(ask_volume, limit - position, self.MAX_TAKE_PER_LEVEL)
                elif z < -1.25 and ask <= fair and position < limit and regime >= 0:
                    qty = min(ask_volume, limit - position, self.MAX_TAKE_PER_LEVEL)
        ...
        if order_depth.buy_orders:
            for bid in sorted(order_depth.buy_orders.keys(), reverse=True):
                ...
                elif z > 1.25 and bid >= fair and position > -limit:
                    qty = min(bid_volume, limit + position, self.MAX_TAKE_PER_LEVEL)
